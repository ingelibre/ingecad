...
NVIDIA Quadro P600), against IngeCAD 0.6.0 and IngeTrazo 0.3.15 alike:
...
X11 without a murmur, so the machine runs IngeCAD perfectly; it just never
got the chance, because nothing fell back. And because the request is the
*default* format, it poisons Qt's own backing store too (``Failed to create
QRhi for QBackingStoreRhiSupport``), so what the user sees is not a broken
canvas — it is an app that does not open.
...
1. **Ask for less.** Multisampling is the part of our format least likely
   to have a matching config, and it is a comfort (Options ▸ Display's line
   smoothing), not a requirement — the canvas draws correctly without it.
...
creation, discarded. Kept deliberately in step with IngeTrazo's copy.
"""
...
def _restart_under_xcb(platform_name: str) -> str:
    ...
    print("IngeCAD: this driver cannot create an OpenGL context under "
...
def ensure_gl_context(app) -> str:
    ...
    ``reduced``     it works without multisampling, now the default;
    ...
    if lean is not None and _can_create(lean):
        ...
        print("IngeCAD: this driver refused the full OpenGL format; "
              "continuing without line smoothing (Options > Display).",
              file=sys.stderr)
        return "reduced"

    outcome = _restart_under_xcb(app.platformName())
    if outcome == "no-x11":
        print("IngeCAD: this driver cannot create an OpenGL 3.3 context "
              "under Wayland, and no X11 display is available to fall back "
              "on. Under Flatpak, grant it X11:\n"
              "  flatpak override --user --socket=x11 org.ingecad.IngeCAD",
              file=sys.stderr)
    return outcome
